...
const navItems = [
  ...
  { path: '/chat', label: 'AI', icon: (
    <svg width="20" height="20" viewBox="0 0 20 20" fill="none">
      <path d="M3 4h14a1 1 0 011 1v8a1 1 0 01-1 1H6l-4 3V5a1 1 0 011-1z" stroke="currentColor" strokeWidth="1.4" fill="none"/>
  ...
  { path: '/analytics', label: 'Stats', icon: (
    <svg width="20" height="20" viewBox="0 0 20 20" fill="none">
      ...
      <rect x="14" y="4" width="4" height="14" rx="1" fill="currentColor" opacity="0.95"/>
    </svg>
  )},
...
const bottomItems = [
  { path: '/profile', label: 'Profile', icon: (
    <svg width="20" height="20" viewBox="0 0 20 20" fill="none">
      ...
      <path d="M3 17c0-3.9 3.1-7 7-7s7 3.1 7 7" stroke="currentColor" strokeWidth="1.4" strokeLinecap="round" fill="none"/>
    </svg>
  )},
  { path: '/settings', label: 'Settings', icon: (
    <svg width="20" height="20" viewBox="0 0 20 20" fill="none">
      <circle cx="10" cy="10" r="2.5" stroke="currentColor" strokeWidth="1.3"/>
      <path d="M10 2v2M10 16v2M2 10h2M16 10h2M4.2 4.2l1.4 1.4M14.4 14.4l1.4 1.4M4.2 15.8l1.4-1.4M14.4 5.6l1.4-1.4" stroke="currentColor" strokeWidth="1.2" strokeLinecap="round"/>
    </svg>
  )},
...
export default function Sidebar() {
  ...
  const NavItem = ({ path, label, icon }) => {
    ...
    return (
      ...
      >
        {icon}
        <span style={{ fontSize: 7.5, fontWeight: active ? 700 : 500, letterSpacing: '.02em' }}>{label}</span>
      </button>
    )
  }
  ...
  return (
    <div className="sidebar" style={{ background: theme.sidebar }}>
      <div
        style={{ width: 42, height: 42, background: theme.accent, borderRadius: 12,
